...
let l =
  ...
  Cons(
    ...
    Cons(
      6,
      @implicit_arity
      Cons(15, @implicit_arity Cons(17, @implicit_arity Cons(15, @implicit_arity Cons(8, Empty)))),
    ),
  )
...
/* 65 */
let in_range_c = ((small, large), x) => less_than((small, x)) && less_than((x, large))

/* 79 */
let rec subst_c = (pred, x) =>
  switch x {
  | (n, Empty) => Empty
...
let rec combine2 = x =>
  ...
  | (Cons(a, l1), Cons(b, l2)) => Cons(a, combine2((l1, Cons(b, l2))))
  }

let rec combine = x =>
  switch x {
  | (Empty, l2) => l2
  | (@implicit_arity Cons(a, l1), l2) => @implicit_arity Cons(a, combine((l1, l2)))
  }
...
let rec combine_c = x =>
  switch x {
  | Empty => l2 => l2
  | @implicit_arity Cons(a, l1) => l2 => @implicit_arity Cons(a, combine_c(l1, l2))
  }

/* 95 */
let prefixer_123 = l2 => @implicit_arity
Cons(1, @implicit_arity Cons(2, @implicit_arity Cons(3, l2)))

/* 97 */
let waiting_prefix_123 = l2 => @implicit_arity
Cons(1, combine_c(@implicit_arity Cons(2, @implicit_arity Cons(3, Empty)), l2))
